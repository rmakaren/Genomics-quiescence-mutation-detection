...
#Data input
###############################################
df <- read.table("final_variants_and_table_total.txt", header =  T, stringsAsFactors=FALSE)

df <- df %>% select(Chrom, Position, Ref, VarFreq, VarAllele, Sample, mutation_type, gene, strain_direction, 
              cDNA, annotation, gene_order, time_in_quiescence, persentage)

#adding manually the variants from WGS
tmp1 <- c("I", 207840, "G", 0.000, "C", "culture_0", "SNP", "sty1" , "-", 74, "sty1-G74C", 4, "3_months", "0%") #sty1 49PP(6R)#
tmp2 <- c("I", 2714003, "T", 0.000, "-AACCCACAA", "culture_0", "Deletion", "pmc1", "-", 2920, "pmc1-2920-9bp", 8, "3_months", "0%") #pmc1 38AP(10W)
tmp3 <- c("I", 613910, "T", 0.000, "-TATGCTTAGA", "culture_0", "Deletion", "mkh1", "-", 2929, "mkh1-2929-10bp", 5, "3_months",  "0%") #mkh1 48AA(14W)
tmp4 <- c("II", 4313731, "T", 0.000, "A", "culture_0", "SNP", "pek1", "-", 969, "pek1-A969T", 6, "3_months", "0%") #pek1 51AA(15W)
tmp5 <- c("II", 731111, "T", 0.000, "+A", "culture_0", "Insertion", "pmk1", "+", 153, "pmk1-153+A", 7, "3_months", "0%") #pmk1 55AA(18W)
tmp6 <- c("I", 2715686, "C", 0.000, "A", "culture_0", "SNP", "pmc1",  "-", 1247, "pmk1-C1247A", 7, "3_months", "0%") #pmc1 60AP(19W)
#tmp7 <- c("II", -713, "A", NA, NA, NA, 0.000, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, "+T", "culture_0", "Insertion", "wis1", "-", "wis1-(-713)+T", 3, "3_months", "0%") #wis1 B(26W)
tmp8 <- c("I", 614355, "C", 0.000, "T", "culture_0", "SNP",  "mkh1", "-", 2494, "mkh1-C2494T", 5, "3_months", "0%") #mkh1 F(30W)
tmp9 <- c("I", 2716606, "A", 0.000, "T", "culture_0", "SNP", "pmc1", "-", 326, "pmc1-A326T", 8, "3_months", "0%") #pmc1 7PP(1R)
tmp10 <- c("I", 2714025, "A", 0.000, "-G", "culture_0", "Deletion", "pmc1", "-", 2906, "pmc1-2906-G", 8, "3_months", "0%") #pmc1 24PP(2R)
tmp11 <- c("I", 2716550, "G", 0.000, "A", "culture_0", "SNP",  "pmc1", "-", 382, "pmc1-G382A", 8, "3_months", "0%") #pmc1 71PP(8R)
tmp12 <- c("I", 2714491, "G", 0.000, "+AATATTATCACCAGTAAC", "culture_0", "Insertion", "pmc1", "-", 2441, "pmc1-2441+18bp", 8, "3_months", "0%") #pmc1 I(33W)
tmp13 <- c("I", 5091526, "G", 0.000, "-G", "culture_0", "Insertion", "win1", "+", 1086, "win1-1086-G", 6, "3_months", "0%") #win1 76AA(24W)

df <- rbind(df, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp8, tmp9, tmp10,
            tmp11, tmp12, tmp13)

#df <- read.table(file = "liste_mutations_totale.txt", sep="\t", header = T, stringsAsFactors = F)
...
df$cDNA <- as.numeric(df$cDNA)

df <- df[order(df$Sample, df$time_in_quiescence, df$gene_order, df$cDNA),]

#removing hotspots
unique_variants <- function (x){
  tmp <- ddply(x, .(Position, gene), nrow)
  tmp2 <- merge(tmp, x, by=c("Position", "gene"))
  x <- tmp2[!duplicated(tmp2[, 1:2]),]
  return(x)
}

#tmp <- ddply(df, .(Position, gene), nrow)
#tmp2 <- merge(tmp, df, by=c("Position", "gene"))
#tmp3 <- tmp2[(duplicated(tmp2[, 1:2], fromLast=T) & !duplicated(tmp2[, 1:2])),]
#test <- tmp2[!duplicated(tmp2[, 1:2]),]
df <- unique_variants(df)
...
df[which(df$annotation_lolliplot == 'CT1884TG' & df$gene == 'mkh1'), "color"] <- "yellow" # mkh1-CT1884TG mkh1-F629V 
df[which(df$annotation_lolliplot == 'A1884C' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-T1884G mkh1-F629V 
df[which(df$annotation_lolliplot == 'G2378C' & df$gene == 'mkh1'),"color"] <- "black" # mkh1-C2378G mkh1-793*
df[which(df$annotation_lolliplot == 'C2494T' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-T2494C mkh1-G974R
df[which(df$annotation_lolliplot == 'A2914G' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-T2914C mkh1-S974P
df[which(df$annotation_lolliplot == 'C3124A' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-G3124T mkh1-A1042S
df[which(df$annotation_lolliplot == 'A3137C' & df$gene == 'mkh1'),"color"] <- "black" # mkh1-T3137G mkh1-1045*
df[which(df$annotation_lolliplot == 'T3138G' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-A3138C mkh1-L1045F
df[which(df$annotation_lolliplot == 'C3145A' & df$gene == 'mkh1'),"color"] <- "black" # mkh1-G3145T mkh1-1049*
df[which(df$annotation_lolliplot == 'T3152C' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-A3152G mkh1-K1057R
df[which(df$annotation_lolliplot == 'T3351A' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-A3351T mkh1-*1117Y
df[which(df$annotation_lolliplot == 'A1885C' & df$gene == 'mkh1'),"color"] <- "yellow" # mkh1-A3351T mkh1-*1117Y

#add mutations that were detected by WGS but not by TR
#updating SNPs for win1
df[which(df$annotation_lolliplot == 'C1751T'),"color"] <- "yellow" # win1-C1751T win1-S1117F
df[which(df$annotation_lolliplot == 'C2350A'),"color"] <- "yellow" # win1-C2350A win1-P1117T
df[which(df$annotation_lolliplot == 'T2381G'),"color"] <- "yellow" # win1-T2381G win1-L1117R

#updating SNPs for pmk1
df[which(df$annotation_lolliplot == 'G497T'),"color"] <- "yellow" # pmk1-G497T pmk1-C166F
df[which(df$annotation_lolliplot == 'T577G'),"color"] <- "yellow" # pmk1-G577T pmk1-W193G
df[which(df$annotation_lolliplot == 'G592A'),"color"] <- "yellow" # pmk1-G592A pmk1-E198K

#updating SNPs for pmc1
df[which(df$annotation_lolliplot == 'C280A'),"color"] <- "black" # pmc1-G280T 
#df[which(df$annotation_lolliplot == 'G280T'),"cDNA"] <- 2434 # pmc1-G280T 
df[which(df$annotation_lolliplot == '2434+18bp'),"annotation"] <- "pmc1-2434+18bp 0%"
df[which(df$annotation_lolliplot == '2434+18bp'),"annotation_lolliplot"] <- "2434+18bp"

#updating SNPs for pek1
df[which(df$annotation_lolliplot == 'T969A'),"color"] <- "black"

#???? df[23,"color"] <- "black" #
#df[which(df$annotation_lolliplot == 'CT1884TG'),"annotation_lolliplot"] <- "CT1884TG"
#df[which(df$annotation_lolliplot == 'CC1281TT'),"annotation_lolliplot"] <- "CC1281TT"
